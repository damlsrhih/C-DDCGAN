...
from LOSS import SSIM_LOSS, L1_LOSS, Fro_LOSS, _tf_fspecial_gauss, combined_loss
...
def train(source_imgs, save_path, EPOCHES_set, BATCH_SIZE, logging_period = 1):
	...
	with tf.Graph().as_default(), tf.compat.v1.Session() as sess:
		LAMBDA = 10
		SOURCE_VIS = tf.compat.v1.placeholder(tf.float32, shape = (BATCH_SIZE, patch_size, patch_size, 1), name = 'SOURCE_VIS')
		SOURCE_IR = tf.compat.v1.placeholder(tf.float32, shape = (BATCH_SIZE, patch_size, patch_size, 1), name = 'SOURCE_IR')
		...
		#######  LOSS FUNCTION
		# Loss for Generator
		G_loss_GAN_D1 = -tf.reduce_mean(tf.compat.v1.log(D1_fake + eps))
		G_loss_GAN_D2 = -tf.reduce_mean(tf.compat.v1.log(D2_fake + eps))
		G_loss_GAN = G_loss_GAN_D1 + G_loss_GAN_D2
		#LOSS_IR = combined_loss(generated_img, SOURCE_IR)
		...
		eps1 = tf.random_uniform([], 0.0, 1.0)
		x_hat = eps1 * SOURCE_VIS + (1 - eps1) * generated_img
		grad_D_x_hat = tf.gradients(D1.discrim(x_hat, reuse=True), [x_hat])[0]
		norm_grad_D = tf.sqrt(tf.reduce_sum(tf.square(grad_D_x_hat), axis=1))
		grad_penalty = tf.reduce_mean(tf.square(norm_grad_D - 1))
		D1_loss_real = -tf.reduce_mean(tf.compat.v1.log(D1_real + eps))
		D1_loss_fake = -tf.reduce_mean(tf.compat.v1.log(1. - D1_fake + eps))
		D1_loss = D1_loss_fake + D1_loss_real + LAMBDA * grad_penalty


		# Loss for Discriminator2
		eps1 = tf.random_uniform([], 0.0, 1.0)
		x_hat = eps1 * SOURCE_IR + (1 - eps1) * generated_img
		grad_D_x_hat = tf.gradients(D2.discrim(x_hat, reuse=True), [x_hat])[0]
		norm_grad_D = tf.sqrt(tf.reduce_sum(tf.square(grad_D_x_hat), axis=1))
		grad_penalty = tf.reduce_mean(tf.square(norm_grad_D - 1))
		D2_loss_real = -tf.reduce_mean(tf.compat.v1.log(D2_real + eps))
		D2_loss_fake = -tf.reduce_mean(tf.compat.v1.log(1. - D2_fake + eps))
		D2_loss = D2_loss_fake + D2_loss_real + LAMBDA * grad_penalty

		current_iter = tf.Variable(0)
		learning_rate = tf.compat.v1.train.exponential_decay(learning_rate = LEARNING_RATE, global_step = current_iter,
		...
		theta_D1 = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope = 'Discriminator1')
		theta_D2 = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope = 'Discriminator2')

		G_GAN_solver = tf.compat.v1.train.RMSPropOptimizer(learning_rate).minimize(G_loss_GAN, global_step = current_iter,
		                                                                 var_list = theta_G)
		G_solver = tf.compat.v1.train.RMSPropOptimizer(learning_rate).minimize(G_loss, global_step = current_iter,
		                                                             var_list = theta_G)
		# G_GAN_solver = tf.train.AdamOptimizer(learning_rate).minimize(G_loss_GAN, global_step = current_iter,
		#                                                                  var_list = theta_G)
		D1_solver = tf.compat.v1.train.GradientDescentOptimizer(learning_rate).minimize(D1_loss, global_step = current_iter,
		                                                                      var_list = theta_D1)
		D2_solver = tf.compat.v1.train.GradientDescentOptimizer(learning_rate).minimize(D2_loss, global_step = current_iter,
		                                                                      var_list = theta_D2)
		...
		#model_path = tf.train.latest_checkpoint('model/third/threetimes/240/')
		# # print(model_path)
		#
