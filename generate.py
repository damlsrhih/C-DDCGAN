...
def generate(ir_path, vis_path, model_path, index, output_path = None):
	...
	with tf.Graph().as_default(), tf.compat.v1.Session() as sess:
		SOURCE_VIS = tf.compat.v1.placeholder(tf.float32, shape = vis_dimension, name = 'SOURCE_VIS')
		SOURCE_ir = tf.compat.v1.placeholder(tf.float32, shape = ir_dimension, name = 'SOURCE_ir')
