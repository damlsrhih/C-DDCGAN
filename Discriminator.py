...
class Discriminator1(object):
	def __init__(self, scope_name):
		...
		with tf.compat.v1.variable_scope(scope_name):
			self.weight_vars.append(self._create_variables(1, 8, 3, scope='conv1'))
			self.weight_vars.append(self._create_variables(8, 16, 3, scope='conv2'))
			self.weight_vars.append(self._create_variables(16, 24, 3, scope='conv3'))
			self.weight_vars.append(self._create_variables(24, 32, 3, scope='conv4'))
			self.weight_vars.append(self._create_variables(32, 64, 3, scope='conv5'))
	...
	def discrim(self, img, reuse):
		...
		with tf.compat.v1.variable_scope(self.scope):
			with tf.compat.v1.variable_scope('flatten1'):
				out = tf.compat.v1.layers.dense(out, 1, activation = tf.nn.tanh, use_bias = True, trainable = True,
				                      reuse = reuse)
		out = out / 2 + 0.5
		return out
